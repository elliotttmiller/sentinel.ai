...
class PerformanceLearningSystem:
    ...
    def _update_performance_model(self, path: str, record: Dict[str, Any]):
        ...
        if model["sample_count"] == 0:
            ...
            model["avg_execution_time"] = (1 - alpha) * model[
                "avg_execution_time"
        ...
        if model["sample_count"] == 0:
            ...
            model["success_rate"] = (1 - alpha) * model[
                "success_rate"
    ...
    def _update_user_preferences(self, task_id: str, path: str, satisfaction: float):
        ...
        if satisfaction > 0.8:
            ...
            user_pref.preferred_path = (
                "full_workflow" if path == "golden_path" else "golden_path"
            )

        user_pref.last_updated = datetime.now()

    def predict_performance(
        self, prompt: str, complexity_score: float
        ...
        return PerformancePrediction(
            ...
            confidence=min(golden_model["sample_count"], full_model["sample_count"])
            / 100.0,
        )
...
class HybridDecisionEngine:
    ...
    def make_hybrid_decision(
        self, prompt: str, user_id: str = "default", force_path: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        decision = {
            ...
            "predicted_time": (
                performance.golden_path_time
                if chosen_path == "golden_path"
                else performance.full_workflow_time
            ),
        ...
        # Log decision
        logger.info(
            f"🎯 Hybrid Decision: {chosen_path} ({reason}) - Confidence: {complexity.confidence:.2f}"
        )
    ...
    def _calculate_decision_score(
        ...
        golden_advantage = performance.full_workflow_time / max(
            performance.golden_path_time, 0.1
        )
...
class TaskComplexityAnalyzer:
    ...
    def analyze(self, prompt: str) -> TaskComplexity:
        ...
        prompt_lower = prompt.lower()
        simple_matches = sum(
            1 for keyword in settings.SIMPLE_TASK_KEYWORDS if keyword in prompt_lower
        )
        complex_matches = sum(
            1 for keyword in settings.COMPLEX_TASK_KEYWORDS if keyword in prompt_lower
        )
        ...
        confidence = min(
            (simple_matches + complex_matches) / 10.0 + (length_score * 0.5), 1.0
        )
...
class AdvancedAnalytics:
    ...
    def record_execution_result(
        ...
        self.user_satisfaction[path].append(
            {"timestamp": timestamp, "satisfaction": user_satisfaction}
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.decision_metrics["total_decisions"]:
            ...
            stats["decision_metrics"]["avg_confidence"] = np.mean(
                [d["confidence"] for d in decisions]
            )
            stats["decision_metrics"]["avg_complexity"] = np.mean(
                [d["complexity_score"] for d in decisions]
            )
                ...
                if executions:
                    stats["execution_metrics"][path] = {
                        ...
                        "success_rate": np.mean(
                            [1.0 if e["success"] else 0.0 for e in executions]
                        ),
                        "avg_satisfaction": np.mean(
                            [e["user_satisfaction"] for e in executions]
                        ),
                    }
