...
class CognitiveForgeEngine:
    ...
    def run_mission(self, user_prompt: str, mission_id_str: str, agent_type: str, 
                   update_callback: Callable[[str], None]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.db_manager.update_mission_status(
                mission_id_str, "planning", plan=plan
            )
    ...
    def _generate_execution_plan(self, user_prompt: str, mission_id_str: str, 
                                update_callback: Callable[[str], None]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            update_callback(f"  ✅ Execution plan generated successfully with {len(plan.get('steps', []))} steps")
            return plan
    ...
    def _execute_worker_crew(self, plan: Dict[str, Any], mission_id_str: str,
                            update_callback: Callable[[str], None]) -> str:
        """
        ...
        for i, step in enumerate(plan.get("steps", [])):
            ...
            if agent_role in agents_map:
                step_number = i + 1
                update_callback(f"  📋 Step {step_number}: Assigning task to [{agent_role.replace('_', ' ').title()}]")
    ...
    def _synthesize_memory(self, mission_id_str: str, prompt: str, result: str, 
                          success: bool) -> None:
        """
