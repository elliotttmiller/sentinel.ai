...
try:
    from src.utils.google_ai_wrapper import (
...
class CognitiveForgeEngine:
    ...
    async def run_mission(
        ...
        transaction = start_transaction(
            f"mission_execution_{mission_id_str}", "mission"
        )
        ...
        try:
            if not is_healing_attempt:
                ...
                self.db_manager.update_mission_status(
                    mission_id_str=mission_id_str, status="running", progress=5
                )
                agent_observability.push_event(
                    LiveStreamEvent(
                        event_type="mission_update",
                        source="mission_control",
                        severity="INFO",
                        message=f"Mission '{mission_id_str}' initiated.",
                        payload=self.db_manager.get_mission(
                            mission_id_str
                        ).as_dict(),  # Send full mission object
                    )
                )
            ...
            self.db_manager.update_mission_status(
                mission_id_str=mission_id_str, status="running", progress=25
            )
            agent_observability.push_event(
                LiveStreamEvent(
                    event_type="mission_update",
            ...
            self.db_manager.update_mission_status(
                mission_id_str=mission_id_str, status="running", progress=50
            )
            agent_observability.push_event(
                LiveStreamEvent(
                    event_type="mission_update",
            ...
            if (
                not is_healing_attempt and random.random() < 0.3
            ):  # 30% chance of failure for testing
                raise ValueError(
                    "Simulated failure: Critical component 'CodeGenerator' failed."
                )
            ...
            self.db_manager.update_mission_status(
                mission_id_str=mission_id_str, status="running", progress=75
            )
            agent_observability.push_event(
                LiveStreamEvent(
                    event_type="mission_update",
            ...
            await self.self_learning_module.synthesize_and_learn(
                self.db_manager.get_mission(mission_id_str)
            )
            ...
            if retries <= 2:  # Allow up to 2 healing attempts
                logger.warning(
                    f"🔥 Phoenix Protocol: Activating self-healing for mission {mission_id_str}. Attempt #{retries}."
                )
                self.db_manager.update_mission_status(
                    mission_id_str=mission_id_str, status="healing", is_healing=True
                )
                agent_observability.push_event(
                    LiveStreamEvent(
                        event_type="mission_update",
                        source="phoenix_protocol",
                        severity="WARNING",
                ...
                logger.info(
                    f"Phoenix Protocol: Rerunning mission with modified prompt."
                )
                # Recursively call the mission with the fix
                await self.run_mission(
                    healed_prompt, mission_id_str, agent_type, is_healing_attempt=True
                )
            else:
                logger.error(
                    f"Mission {mission_id_str} failed permanently after {retries} healing attempts."
                )
    ...
    def get_mission_status(self) -> Dict[str, Any]:
        ...
        return {
            "active_missions": stats.get("total_missions", 0)
            - stats.get("completed_missions", 0)
            - stats.get("failed_missions", 0),
