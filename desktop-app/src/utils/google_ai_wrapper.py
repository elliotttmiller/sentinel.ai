...
from langchain_core.pydantic_v1 import (  # CRITICAL: Use langchain's pydantic_v1
    Field,
    PrivateAttr,
)
...
class GoogleGenerativeAIWrapper(BaseChatModel):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            ...
            logger.info(
                f"API Key found: {api_key[:10]}..." if api_key else "No API key found"
            )
            ...
            logger.critical(
                f"Fatal error initializing Google Generative AI Wrapper: {e}"
            )
            raise
    ...
    def model(self):
        ...
        if self._model is None:
            try:
                ...
                logger.debug(
                    f"Google Generative AI model created with: {model_name}, temp={temperature_val}"
                )
        ...
        **kwargs: Any,
        ...
        try:
            ...
            yield ChatGeneration(
                message=AIMessage(content=f"An error occurred during streaming: {e}")
            )
...
class GoogleAIWrapper:
    ...
    async def direct_inference(self, prompt: str, system_context: str = "") -> str:
        ...
        try:
            if settings.GOLDEN_PATH_LOGGING:
                logger.info(
                    f"🟡 Golden Path: Direct inference for prompt: {prompt[:100]}..."
                )
    ...
    async def structured_inference(
        self, prompt: str, system_context: str = "", temperature: float = 0.7
    ) -> Dict[str, Any]:
        """
