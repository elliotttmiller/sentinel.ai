...
class GuardianProtocol:
    ...
    async def run_pre_flight_check(self, prompt: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"🛡️ Guardian Protocol: Pre-flight check complete. Go/No-Go: {analysis['go_no_go']}. Feedback: {analysis['feedback']}"
        )
        return analysis
    ...
    def _calculate_clarity_score(self, prompt_lower: str) -> float:
        ...
        specific_count = sum(
            1
            for phrase in self.clarity_indicators["specific_actions"]
            if phrase in prompt_lower
        )
        technical_count = sum(
            1
            for phrase in self.clarity_indicators["technical_specificity"]
            if phrase in prompt_lower
        )
        ...
        vague_count = sum(
            1
            for phrase in self.clarity_indicators["vague_phrases"]
            if phrase in prompt_lower
        )

        # Calculate score
        score += (
            (specific_count * 0.1) + (technical_count * 0.05) - (vague_count * 0.15)
        )
    ...
    def _calculate_risk_score(self, prompt_lower: str) -> float:
        ...
        high_risk_count = sum(
            1 for pattern in self.risk_patterns["high_risk"] if pattern in prompt_lower
        )
        ...
        low_risk_count = sum(
            1 for pattern in self.risk_patterns["low_risk"] if pattern in prompt_lower
        )
    ...
    def _generate_feedback_and_suggestions(
        ...
        if analysis["risk_score"] > 0.6:
            ...
            analysis["feedback"] = (
                "🚨 HIGH RISK: This operation contains potentially dangerous commands. Please specify actions in a safe, sandboxed environment."
            )
            analysis["suggestions"].append(
                "Consider using a development or testing environment"
            )
            analysis["suggestions"].append("Add safety checks and validation steps")
            analysis["suggestions"].append(
                "Specify exact files or systems to be modified"
            )

        elif analysis["risk_score"] > 0.3:
            analysis["go_no_go"] = True
            analysis["feedback"] = (
                "⚠️ MEDIUM RISK: This operation involves system changes. Proceed with caution."
            )
            analysis["suggestions"].append(
            ...
            analysis["go_no_go"] = False
            analysis["feedback"] = (
                "❓ UNCLEAR: The prompt is too ambiguous. Please provide more specific instructions."
            )
            analysis["suggestions"].append(
                "Try defining a clear function or class to be created"
            )
            analysis["suggestions"].append("Specify the expected inputs and outputs")
            analysis["suggestions"].append(
                "Include specific file names or system components"
            )
            analysis["suggestions"].append(
                "Describe the exact problem you want to solve"
            )

        elif analysis["clarity_score"] < 0.6:
            analysis["go_no_go"] = True
            analysis["feedback"] = (
                "📝 FAIR: The prompt could be more specific, but is acceptable for execution."
            )
            ...
            analysis["go_no_go"] = True
            analysis["feedback"] = (
                "✅ CLEAR & SAFE: Prompt is well-defined and within safe operational parameters. Ready for launch."
            )

            if analysis["clarity_score"] > 0.8:
                analysis["suggestions"].append(
                    "Excellent prompt clarity - execution should be smooth"
                )
        ...
        if agent_type == "developer":
            if config.get("temperature", 0.7) > 0.8:
                validation_results["warnings"].append(
                    "High temperature may lead to inconsistent outputs"
                )
    ...
    def get_risk_assessment_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_patterns": sum(
                len(patterns) for patterns in self.risk_patterns.values()
            ),
        }
