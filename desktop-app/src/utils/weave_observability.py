...
class WeaveObservabilityManager:
    ...
    def _record_agent_metrics(self, metrics: AgentMetrics):
        ...
        if metrics.agent_name not in self.performance_baselines:
            ...
            baseline["avg_execution_time"] = (
                (baseline["avg_execution_time"] * baseline["total_executions"] + metrics.execution_time) /
                (baseline["total_executions"] + 1)
            )
            baseline["success_rate"] = (
                (baseline["success_rate"] * baseline["total_executions"] + (1.0 if metrics.success else 0.0)) /
                (baseline["total_executions"] + 1)
            )
            baseline["total_executions"] += 1
        ...
        if self.wandb_run:
            wandb.log({
                ...
                f"agent_{metrics.agent_name}_success": 1.0 if metrics.success else 0.0,
                f"agent_{metrics.agent_name}_tokens": metrics.input_tokens + metrics.output_tokens,
    ...
    def _record_phase_metrics(self, phase_data: Dict[str, Any]):
        ...
        if self.wandb_run:
            wandb.log({
                f"phase_{phase_data['phase_name']}_duration": phase_data["duration"],
                f"phase_{phase_data['phase_name']}_success": 1.0 if phase_data["success"] else 0.0
            })
    
    def log_system_event(self, event_type: str, event_data: Dict[str, Any], mission_id: Optional[str] = None):
    ...
    def log_error(self, error: Exception, context: Dict[str, Any], mission_id: Optional[str] = None):
    ...
    def generate_observability_report(self) -> Dict[str, Any]:
        ...
        report = {
            "timestamp": datetime.now().isoformat(),
            "system_status": "operational" if analytics["system_health"]["success_rate"] > 0.8 else "degraded",
            "analytics": analytics,
...
class WeaveEnhancedAgent:
    ...
    def __init__(self, agent_name: str, original_agent, observability_manager: WeaveObservabilityManager):
    ...
    async def execute(self, task, context: Dict[str, Any], mission_id: str):
        ...
        with self.observability.agent_trace(self.agent_name, mission_id, str(task)) as metrics:
            try:
                ...
                metrics.error_message = str(e)
                self.observability.log_error(e, {"agent": self.agent_name, "task": str(task)}, mission_id)
                raise
...
class WeaveEnhancedEngine:
    ...
    def __init__(self, original_engine, observability_manager: WeaveObservabilityManager):
    ...
    async def run_mission_with_observability(self, user_request: str) -> Dict[str, Any]:
        ...
        with self.observability.mission_trace(mission_id, user_request) as trace_data:
            try:
                ...
                with self.observability.phase_trace("prompt_optimization", mission_id) as phase_data:
                    optimized_result = await self.original_engine._execute_prompt_alchemy(user_request)
                    phase_data["success"] = True
                    trace_data.phases.append(phase_data)
                
                # Phase 2: Planning Specialist
                with self.observability.phase_trace("planning_specialist", mission_id) as phase_data:
                    execution_blueprint = await self.original_engine._execute_planning_specialist(
                ...
                with self.observability.phase_trace("blueprint_validation", mission_id) as phase_data:
                    validation_result = await self.original_engine._validate_execution_blueprint(execution_blueprint)
                    phase_data["success"] = True
                    trace_data.phases.append(phase_data)
                
                # Phase 4: Mission Execution
                with self.observability.phase_trace("mission_execution", mission_id) as phase_data:
                    execution_result = await self.original_engine._execute_mission_from_blueprint(
