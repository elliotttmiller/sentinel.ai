...
from src.utils.weave_observability import (
...
class WeaveEnhancedFixAI:
        ...
        if LLM_AVAILABLE:
            try:
                ...
                self.llm = create_google_ai_llm(
                    model=os.getenv("LLM_MODEL", "gemini-1.5-pro"), temperature=0.3
                )
    ...
    async def run_comprehensive_healing(self) -> Dict[str, Any]:
        ...
        with self.observability.mission_trace(
            self.operation_id, "Comprehensive Codebase Healing"
        ) as trace_data:
            try:
                # Phase 1: Diagnosis with observability
                with self.observability.phase_trace(
                    "diagnosis", self.operation_id
                ) as phase_data:
                ...
                with self.observability.phase_trace(
                    "performance_analysis", self.operation_id
                ) as phase_data:
                    performance_result = await self._run_performance_analysis_phase()
                    phase_data["success"] = True
                    phase_data["performance_issues"] = len(
                        performance_result.get("performance_issues", [])
                    )
                    metrics.performance_improvements.extend(
                        performance_result.get("optimization_suggestions", [])
                    )
                    trace_data.phases.append(phase_data)

                # Phase 3: Triage & Planning with observability
                with self.observability.phase_trace(
                    "triage_planning", self.operation_id
                ) as phase_data:
                    planning_result = await self._run_planning_phase(
                        diagnosis_result, performance_result
                    )
                    phase_data["success"] = True
                    phase_data["healing_steps"] = len(
                        planning_result.get("healing_plan", [])
                    )
                    trace_data.phases.append(phase_data)

                # Phase 4: Execution & Self-Healing with observability
                with self.observability.phase_trace(
                    "execution_self_healing", self.operation_id
                ) as phase_data:
                    ...
                    metrics.surgical_fixes = execution_result.get(
                        "surgical_fixes_applied", 0
                    )
                    metrics.simple_fixes = execution_result.get(
                        "simple_fixes_applied", 0
                    )
                    metrics.rollbacks_performed = execution_result.get(
                ...
                with self.observability.phase_trace(
                    "final_validation", self.operation_id
                ) as phase_data:
                    validation_result = await self._run_final_validation_phase()
                    phase_data["success"] = validation_result.get(
                        "all_tests_passed", False
                    )
                    phase_data["regressions_found"] = len(
                        validation_result.get("regressions", [])
                    )
                    trace_data.phases.append(phase_data)

                metrics.end_time = datetime.now()
                metrics.success_rate = (
                    1.0 if validation_result.get("all_tests_passed", False) else 0.0
                )
                metrics.total_files_analyzed = diagnosis_result.get(
                ...
                self.observability.log_system_event(
                    "fix_ai_operation_completed", asdict(metrics), self.operation_id
                )
                ...
                self.observability.log_error(
                    e, {"operation_id": self.operation_id}, self.operation_id
                )
    ...
    async def _run_planning_phase(
        self, diagnosis_result: Dict[str, Any], performance_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {"healing_plan": healing_plan, "total_actions": len(healing_plan)}
    ...
    async def _run_execution_phase(
        ...
        for action in planning_result.get("healing_plan", []):
            try:
                ...
                failed_fixes += 1
                logger.error(
                    f"Failed to apply {action['type']} to {action['target']}: {e}"
                )
