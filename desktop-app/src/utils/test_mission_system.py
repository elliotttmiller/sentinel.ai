...
from src.utils.agent_observability import AgentSession, agent_observability
...
class TestMission:
    ...
    mission_id: str = field(
        default_factory=lambda: f"test_mission_{uuid.uuid4().hex[:8]}"
    )
    name: str = "Unknown Test Mission"
    description: str = "No description"
    category: str = (
        "general"  # 'code_review', 'bug_fix', 'feature_dev', 'optimization', 'analysis'
    )
...
def display_user_input(user_input):
    ...
    async def run_test_mission(
        self, mission_id: str, user_request: str = None
    ) -> TestExecution:
            ...
            try:
                ...
                test_start_event = agent_observability._create_live_stream_event(
                    ...
                    event_data={
                        ...
                        or f"Test Mission: {test_mission.name}",
                    },
                    severity="info",
                ...
                for i, scenario in enumerate(test_mission.test_scenarios):
                    ...
                    scenario_start_event = (
                        agent_observability._create_live_stream_event(
                    ...
                    test_result = await self._execute_test_scenario_with_real_agents(
                        scenario, execution.execution_id
                    )
                ...
                execution.success = self._evaluate_test_success(
                    execution, test_mission.success_criteria
                )
                execution.performance_metrics = self._calculate_execution_performance(
                    execution
                )
                ...
                logger.info(
                    f"✅ Test mission completed: {test_mission.name} - Success: {execution.success}"
                )
    ...
    async def _execute_test_scenario_with_real_agents(
        self, scenario: Dict[str, Any], execution_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            if not self.cognitive_engine:
                raise ValueError(
                    "Cognitive engine not available for real agent execution"
                )
            ...
            result = await self.cognitive_engine.run_mission_simple(
                scenario_prompt, execution_id
            )
    ...
    def _evaluate_test_success(
        self, execution: TestExecution, criteria: Dict[str, Any]
    ) -> bool:
        ...
        all_scenarios_successful = all(
            result.get("success", False) for result in execution.test_results
        )

        # Check specific criteria
        if "min_issues_detected" in criteria:
            total_issues = sum(
                result.get("issues_found", 0) for result in execution.test_results
            )
    ...
    def _calculate_execution_performance(
        ...
        return {
            ...
            "successful_scenarios": sum(
                1 for r in execution.test_results if r.get("success", False)
            ),
            "avg_scenario_duration_ms": sum(
                r.get("duration_ms", 0) for r in execution.test_results
            )
            / len(execution.test_results),
            "total_issues_found": sum(
                r.get("issues_found", 0) for r in execution.test_results
            ),
            "total_suggestions": sum(
                len(r.get("suggestions", [])) for r in execution.test_results
            ),
            ...
            / len(execution.test_results),
            "avg_score": sum(
                r.get("analysis", {}).get("score", 0) for r in execution.test_results
            )
            / len(execution.test_results),
        }
    ...
    def get_test_execution_history(self) -> List[Dict[str, Any]]:
        ...
        return [
            self._make_execution_serializable(execution)
    ...
    def get_agent_performance_analysis(self) -> Dict[str, Any]:
        ...
        avg_duration = (
            sum(e.duration_seconds for e in self.executions.values()) / total_executions
        )
        avg_success_rate = (
            sum(
                sum(1 for r in e.test_results if r.get("success", False))
                / len(e.test_results)
