...
class LogCollector:
    ...
    async def start_collection(self):
        ...
        for server_port, server_url in self.servers.items():
            task = asyncio.create_task(self._collect_server_logs(server_port, server_url))
            tasks.append(task)
    ...
    async def _collect_server_logs(self, server_port: str, server_url: str):
        ...
        logger.info(f"📡 Starting real-time log collection from server {server_port} at {server_url}")
    ...
    async def _check_server_health(self, server_port: str, server_url: str) -> Dict[str, Any]:
        """Check if server is online and responsive."""
        try:
            async with aiohttp.ClientSession() as session:
                # Try health endpoint first
                try:
                    async with session.get(f"{server_url}/health", timeout=3) as response:
                    ...
                    try:
                        async with session.get(endpoint, timeout=5) as response:
                            if response.status == 200:
                                ...
                                await self._process_real_logs(server_port, data, endpoint)
                                logs_collected = True
                                break
                    except Exception as e:
                        continue
    ...
    async def _process_real_logs(self, server_port: str, data: Dict[str, Any], endpoint: str):
        ...
        try:
            ...
            for log_entry in logs:
                if isinstance(log_entry, dict):
                    ...
                    processed_log = {
                        "message": log_entry.get("message", f"Server {server_port} activity"),
...
class SimpleLogAggregator:
    ...
    def get_aggregated_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "info_count": 10,
            "server_status": {
                "8001": {"status": "online", "last_seen": datetime.utcnow().isoformat()},
                "8002": {"status": "online", "last_seen": datetime.utcnow().isoformat()}
            },
            "last_updated": datetime.utcnow().isoformat()
        } 