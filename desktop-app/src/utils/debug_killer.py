...
def execute_solution(solution: DebugSolution) -> bool:
    ...
    try:
        ...
        if solution.solution.startswith("Install missing package:"):
            ...
            result = subprocess.run([sys.executable, "-m", "pip", "install", "onnxruntime-cpu"], 
                                  capture_output=True, text=True, timeout=60)
            success = result.returncode == 0
            ...
            result = subprocess.run([sys.executable, "-m", "pip", "install", "onnxruntime==1.16.3"], 
                                  capture_output=True, text=True, timeout=60)
            success = result.returncode == 0
...
def debug_killer_main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            for port in common_ports:
                if port in [conn.laddr.port for proc in psutil.process_iter(["connections"]) 
                           for conn in proc.info.get("connections", [])]:
