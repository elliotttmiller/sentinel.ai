...
class SentryAPIClient:
    ...
    def __init__(
        self, org_slug: Optional[str] = None, project_id: Optional[str] = None
    ):
    ...
    def get_recent_issues(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        if not self.auth_token or not self.org_slug or not self.project_id:
            logger.warning(
                "Sentry API credentials not configured. Using simulated data."
            )
        ...
        try:
            ...
            # Use minimal query parameters to avoid 400 errors
            params = {"limit": 25}  # Reduced limit to avoid API issues

            # Add statsPeriod only if hours > 0
            if hours > 0:
                params["statsPeriod"] = f"{hours}h"
