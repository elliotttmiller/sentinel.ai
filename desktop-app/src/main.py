...
try:
    ...
    from src.models.advanced_database import User, db_manager
    from src.utils.agent_observability import agent_observability
...
if db_manager is None:
    ...
    class FallbackDatabaseManager:
        ...
        def get_mission(self, mission_id_str):
            return type(
                "Mission", (), {"as_dict": lambda: {"mission_id_str": mission_id_str}}
            )()
        ...
        def update_proposal_status(self, proposal_id, status):
            return type(
                "Proposal",
                (),
                {"as_dict": lambda: {"id": proposal_id, "status": status}},
            )()
...
async def pre_flight_check(request: Request):
    ...
    try:
        ...
        if cognitive_forge_engine and cognitive_forge_engine.guardian_protocol:
            analysis = (
                await cognitive_forge_engine.guardian_protocol.run_pre_flight_check(
                    prompt
                )
            )
        else:
...
async def create_test_mission(
    request: TestMissionRequest, current_user: User = Depends(get_current_user)
):
    """Create a test mission with enhanced error handling"""
    try:
        logger.info(
            f"🧪 Creating test mission: {request.test_type} - {request.prompt[:50]}..."
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create test mission: {str(e)}"
        )
...
async def get_performance_over_time(current_user: User = Depends(get_current_user)):
    ...
    try:
        performance_data = db_manager.get_performance_data_for_analytics(
...
async def apply_optimization(proposal_id: int):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to apply optimization: {str(e)}"
        )
...
async def reject_optimization(proposal_id: int):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to reject optimization: {str(e)}"
        )
...
async def startup_event():
    ...
    logger.info(
        "🚀 Sentinel Cognitive Forge v5.4 (Predictive Intelligence Activated) starting up..."
    )
